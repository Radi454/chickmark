...
// NATURAL_DATA_ENTRY, BENCHMARK_DISCIPLINE, TOOL_DISCIPLINE). This test pins the
...
// inserted between EVIDENCE_AND_SCOPE and NATURAL_DATA_ENTRY.
...
Deno.test('the grounding guard section is present in the typed policy', () => {
...
Deno.test('the typed policy does not carry a voice-only tool-result section', () => {
  const hasToolResults = CHICKMARK_AGENT_POLICY.includes('Tool results:')
  assertEquals(hasToolResults, false)
})

Deno.test('the typed policy does not carry a voice-only report-vs-benchmark section', () => {
