...
Deno.test('agent policy captures the natural conversation acceptance contract', () => {
  for (
    const requirement of [
      ...
      'number it as option 1',
      'never ask for a yes/no confirmation',
      'let the user choose before loading a summary',
      'bare numbered choice',
      'affirmative confirmation after offering the only audit',
      'call select_audit_option',
      'Do not re-resolve or re-list the customer or flock',
      'never pass an audit ID to get_audit_summary',
      'get_audit_summary only reloads the already selected audit',
      'Never reconstruct or re-list an ordinal mapping',
      'call get_selected_audit_breakouts',
      'Do not claim selected audit breakout data is unavailable before calling that tool',
