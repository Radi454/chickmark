...
class AdminRepository {
  ...
  Future<List<AdminProfile>> listProfiles() async {
    final rows = await _client
        .from('profiles')
        .select('id, email, full_name, role, status, customer_id')
        .order('email');
    return rows
  ...
  Future<void> setAuditorCustomers(
    ...
    await _client.from('auditor_customers').delete().eq('auditor_id', auditorId);
    if (customerIds.isEmpty) return;
    await _client.from('auditor_customers').insert(
          customerIds
