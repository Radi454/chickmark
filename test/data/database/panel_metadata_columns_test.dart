...
void main() {
  ...
  test(
    'every panel table carries the sample and domain metadata columns',
    () async {
      ...
      for (final panel in PanelSampleSchema.panels) {
        ...
        expect(
          ...
          isTrue,
          reason:
              '${panel.tableName} is missing ${expected.difference(columns)}',
        );
      }
    },
  );

  test('database version is 65', () async {
    final db = await DatabaseHelper().db;
