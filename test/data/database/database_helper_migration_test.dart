...
Future<void> _createV35SetterColumnFixture(Database db) async {
  const now = '2026-05-14T08:00:00.000';
  await db.execute('CREATE TABLE users (id TEXT PRIMARY KEY, email TEXT)');
  ...
  await db.insert('users', {'id': 'user-1', 'email': 'auditor@example.com'});
  ...
  await db.insert('egg_quality_samples', {
    'id': 'panel-sample-1',
    'panelId': 'panel-1',
    'scopeType': 'pool',
    'scopeLabel': 'Random',
...
void main() {
  ...
  test('v35 upgrade adds setter machine columns and preserves data', () async {
    final db = await _openInMemoryDatabase();
    addTearDown(db.close);
    await _createV35SetterColumnFixture(db);

    await DatabaseHelper().applyV35UpgradeForTest(db);
    ...
    expect(await db.query('photos'), hasLength(1));
    expect(await db.query('sample_records'), hasLength(1));
    expect(await db.query('sample_house_details'), hasLength(1));
    expect(await db.query('egg_quality'), hasLength(1));
    expect(await db.query('egg_quality_samples'), hasLength(1));
    expect(await db.query('sync_tombstones'), hasLength(1));
    ...
    final logs = await db.query('activity_log', orderBy: 'id ASC');
    expect(logs.map((row) => row['id']), ['log-audit-1', 'log-login-1']);
  });
